...
{
bool checkDuplicateState(const std::vector<std::vector<double>> &states, const std::vector<double> &new_state)
{
...
ob::ScopedState<> vectorToState(ompl::base::StateSpacePtr space, const std::vector<double> &screw_state,
...
ScrewGoal::ScrewGoal(const ob::SpaceInformationPtr si) : GoalStates(si), screw_bounds_(1)
{
    ob::CompoundStateSpace *compound_space = si_->getStateSpace()->as<ob::CompoundStateSpace>();
    screw_bounds_ = compound_space->getSubspace(0)->as<ob::RealVectorStateSpace>()->getBounds();
}
    ...
    : ob::StateValidityChecker(si), robot_bounds_(1), screw_bounds_(1)
{
    ...
    joint_model_group_ =
        std::make_shared<moveit::core::JointModelGroup>(*kinematic_model->getJointModelGroup(mg_string));
    ...
    screw_axis_.setScrewAxis(*affordance_primitives::strToScrewMsg(screw_msg_string));
    tf2::fromMsg(affordance_primitives::strToPose(pose_msg_string)->pose, start_pose_);

    ob::CompoundStateSpace *compound_space = si_->getStateSpace()->as<ob::CompoundStateSpace>();
    screw_bounds_ = compound_space->getSubspace(0)->as<ob::RealVectorStateSpace>()->getBounds();
    robot_bounds_ = compound_space->getSubspace(1)->as<ob::RealVectorStateSpace>()->getBounds();
}
...
bool ScrewValidityChecker::isValid(const ob::State *state) const
{
    ...
    {
        if (screw_state[i] > screw_bounds_.high[i] || screw_state[i] < screw_bounds_.low[i])
        {
            return false;
        }
    }
    ...
    {
        if (robot_state[i] > robot_bounds_.high[i] || robot_state[i] < robot_bounds_.low[i])
        {
            return false;
        }
    ...
    kinematic_state_->setJointGroupPositions(joint_model_group_.get(), joint_state);
    ...
    Eigen::VectorXd phi_low((Eigen::VectorXd() << screw_bounds_.low[0]).finished());
    Eigen::VectorXd phi_high((Eigen::VectorXd() << screw_bounds_.high[0]).finished());
    const std::pair<Eigen::VectorXd, Eigen::VectorXd> phi_bounds = {phi_low, phi_high};
    affordance_primitives::ScrewConstraintInfo screw_constraint_info{phi_bounds, start_pose_, this_state_pose};
